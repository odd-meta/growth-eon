...
function play.draw()
    ...
    if play.environ._building == true then
        ...
        local percent_done = round( ( play.environ._build_counter / ( (play.environ.max_x + 1) * ( play.environ.max_y + 1 )  ) ) * 100 , 0)
...
function play.build_environment()
    ...
    if play.environ._cur_build_x <= play.environ.max_x then
        ...
        if play.environ._cur_build_y <= play.environ.max_y then
            play.environ:add_random( { x = play.environ._cur_build_x, y = play.environ._cur_build_y } )
